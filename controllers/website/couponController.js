...
class CouponController extends BaseController {
      ...
      const coupons = await Coupon.aggregate([
        ...
        },
        {
          ...
          $addFields: {
            isValidCategoryId: {
              $cond: {
                if: {
                  $and: [{ $ne: ['$categoryId', null] }, { $ne: ['$categoryId', ''] }, { $eq: [{ $type: '$categoryId' }, 'objectId'] }],
                },
                then: true,
                else: false,
              },
        ...
        },
        {
          $match: {
            ...
            $or: [{ isValidCategoryId: true }, { categoryId: { $exists: false } }, { categoryId: null }],
          },
        },
        { $sample: { size: 12 } },
        {
  ...
  async offerFilter(req, res, next) {
    try {
      let { advertiserId, advertiserName, status, type, startDatetime, endDatetime, isActive, page = 1, limit = 10, sort = 'createdAt', order = 'desc' } = req.query;
  ...
  async advertisementFilter(req, res, next) {
    try {
      ...
      // ✅ Query
      const advertisements = await Advertisement.find(filters).sort(sortQuery).skip(skip).limit(limitNum).lean();
