...
class BlogController extends BaseController {
  ...
  async insert(req, res, next) {
    try {
      const { _id } = req.user;
      const { categoryId, subCategoryId, name, blog, shortDescription, tag = [] } = req.body;

      // Validate required fields
      if (!categoryId || !subCategoryId || !name || !blog || !shortDescription || !tag) {
        return this.handleError(next, 'All required fields (categoryId, subCategoryId, name, blog, shortDescription, tag) must be provided', 400);
      }
  ...
  async info(req, res, next) {
    try {
      const { id } = req.params;
      const blog = await Blog.findById(id).populate('adminId', 'name').populate('categoryId', 'name').populate('subCategoryId', 'name');
